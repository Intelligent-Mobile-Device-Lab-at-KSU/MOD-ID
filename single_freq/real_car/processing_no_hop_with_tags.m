...
for currSlideLoc = 0:FTDP_adv_samps:L-wind
    ...
    if length(locs) > 2 % if there are more, then this is burst of energy that needs to be removed
       ...
       myfsvals = [myfsvals 0];
        ...
        [pks locs]=findpeaks(obswindow./max(obswindow),'MINPEAKHEIGHT',.7,'MINPEAKDISTANCE',10); %leave these settings
        if length(locs) > 2
...
typ = FTDP_Window(:,cc/2:end);
ampmin=max(max(abs(typ.')))/20;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         ;
imagesc(t_FTDP(1:rr),f_FTDP(cc/2:end),10*log10(max(abs(typ.'),ampmin)/ampmin));%20*log10(max(abs(typ.'),ampmin)/ampmin));
labelsize = 18;
